...
def _resolve_layer_source(
    ...
    if layer_config.source in sources:
        return sources[layer_config.source]
    available = ", ".join(sorted(sources.keys())) if sources else "(none)"
    raise PipelineError(
        f"Layer '{layer_config.id}' references unknown source "
        f"'{layer_config.source}'. Available sources: {available}"
    )
