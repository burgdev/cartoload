...
The system SHALL compute Garmin TRE1 zoom codes dynamically based on the number and order of zoom levels, using the pattern: first level gets code `0x80 + (N-1)`, remaining levels count down from `N-2` to `0`.
